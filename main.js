...
export default async (client, m) => {
  ...
  m.text = body;  // Asignar body a m.text para corregir el error de undefined
  ...
  if (botprimaryId && botprimaryId !== botJid) {
    if (hasPrefix) {
      ...
      const primaryInGroup = participants.some(p => (p.id) === botprimaryId);  // Simplificado a p.id
