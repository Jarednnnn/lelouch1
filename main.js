...
import { jidNormalizedUser } from 'baileys';               // 👈 IMPORTACIÓN AÑADIDA
...
export default async (client, m) => {
  ...
  const botJid = client.user.id;                           // Usamos directamente el JID completo del bot
  ...
  if (m.isGroup) {
    ...
    // 1. Extraer los JIDs de los administradores (pueden venir con @lid)
    const adminJids = groupMetadata?.participants
      .filter(p => p.admin === 'admin' || p.admin === 'superadmin')
      .map(p => p.id) || [];

    // 2. Normalizar todos los JIDs a @s.whatsapp.net
    const normalizedAdmins = adminJids.map(jid => jidNormalizedUser(jid));

    // 3. Normalizar el sender y el bot
    const normalizedSender = jidNormalizedUser(sender);
    const normalizedBotJid = jidNormalizedUser(botJid);

    // 4. Verificar si el sender es admin y si el bot es admin
    isAdmins = normalizedAdmins.includes(normalizedSender);
    isBotAdmins = normalizedAdmins.includes(normalizedBotJid);
  }
  // ========================================================================
