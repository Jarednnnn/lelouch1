...
// ✅ Extrae número limpio de cualquier JID o LID
const extractNumber = (jid) => jid?.split('@')[0]?.split(':')[0] || ''
...
// ✅ Números normalizados para comparación
...
const primaryInGroup = participants.some(p => extractNumber(p.id) === extractNumber(botprimaryId) || extractNumber(p.lid) === extractNumber(botprimaryId))
