...
// ⚠️ CORREGIDO: Limpieza de JID en administradores
export function getGroupAdmins(participants) {
    if (!participants) return []
    // Esta función recorre a todos los miembros y limpia sus IDs
    return participants
...
export async function smsg(client, m, store) {
  ...
  if (m.key) {
   ...
   // ⚠️ CORREGIDO: Limpieza del JID del remitente
   let rawSender = await fixLid(client, m)
   m.sender = rawSender.split('@')[0].split(':')[0] + '@s.whatsapp.net'
   }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      if (m.msg?.contextInfo?.participant?.endsWith('@lid'))
        m.msg.contextInfo.participant =
          ...
          m.msg.contextInfo.participant
      // ⚠️ CORREGIDO: Limpieza del JID del remitente del mensaje citado
      let rawQuotedSender = await fixLid2(client, m)
      m.quoted.sender = rawQuotedSender.split('@')[0].split(':')[0] + '@s.whatsapp.net'
      ...
      m.quoted.key = {
        remoteJid: m.msg?.contextInfo?.remoteJil|| m.chat,
