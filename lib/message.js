...
// --- NUEVA FUNCIÓN getGroupAdmins (normaliza los JIDs) ---
// ... (todo el código previo se mantiene igual)
// Asegúrate de que la función getGroupAdmins esté definida como se muestra:

export function getGroupAdmins(participants) {
  let admins = []
  for (let i of participants) {
    if (i.admin === 'admin' || i.admin === 'superadmin') {
      // Normaliza: quita el sufijo ':' y todo lo que sigue antes de '@'
      let jid = i.id.split('@')[0].split(':')[0] + '@s.whatsapp.net'
      admins.push(jid)
    }
  }
  return admins
...
export async function smsg(client, m, store) {
  ...
  if (m.key) {
   ...
   //  m.sender = client.decodeJid((m.fromMe && client.user.id) || m.key.participant)
     ...
     // --- NUEVA PROPIEDAD m.isAdmin (obtenida en tiempo real) ---
     if (m.isGroup) {
       try {
         const groupMetadata = await client.groupMetadata(m.chat)
         const participants = groupMetadata.participants
         const adminList = getGroupAdmins(participants) // ya normalizados
         const senderNormalized = m.sender.split('@')[0].split(':')[0] + '@s.whatsapp.net'
         m.isAdmin = adminList.includes(senderNormalized)
       } catch (e) {
